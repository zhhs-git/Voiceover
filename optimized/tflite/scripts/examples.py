...
- scripts/sa3_tflite.py — appended to `./sa3 --help`

Examples render with the user's `./sa3` wrapper when present (falling back to
`.venv/bin/python` or bare `python`), and only show entries for DiT bundles that
the user actually has installed. Bundles not yet on disk get a "re-run install.sh,
or just use them — weights auto-download" note at the bottom.
"""
...
def _py_invocation() -> tuple[str, str, bool]:
    ...
    if venv_py.exists():
        return str(venv_py.relative_to(SCRIPT_DIR)), "", False
    if Path(sys.prefix).resolve() == (SCRIPT_DIR / ".venv").resolve():
...
def print_example_commands(header: str | None = None) -> None:
    ...
    prefix = py if is_wrapper else f"{py} scripts/sa3_tflite.py"
    ...
    have = {name: bundle_status(name) == (4, 4) for name in ("sm-music", "sm-sfx", "medium")}

    # ── Basic generation ─────────────────────────────────────────────
    hdr("🎵 Generate audio from a prompt")
    if have["sm-music"]:
        cmd('--prompt "lofi house loop, mellow piano" \\\n'
            f'        --dit sm-music --decoder same-s --seconds 30 --out lofi.wav',
            "fast music generation")
    if have["sm-sfx"]:
        cmd('--prompt "footsteps on gravel, then a door slamming" \\\n'
            f'        --dit sm-sfx --decoder same-s --seconds 20 --out sfx.wav',
    ...
    if have["medium"]:
        cmd('--prompt "A beautiful piano arpeggio grows into a cinematic climax" \\\n'
            ...
            "highest quality (chunked SAME-L decode)")

    # ── Playback ─────────────────────────────────────────────────────
    hdr("▶  Play immediately after generation")
    one_dit = "sm-music" if have["sm-music"] else ("sm-sfx" if have["sm-sfx"] else "medium")
    ...
    cmd(f'--prompt "ambient drone" --cfg 3.0 \\\n'
        ...
        "cfg > 1.0 toward prompt, neg pushes away (sequential dual-pass)")

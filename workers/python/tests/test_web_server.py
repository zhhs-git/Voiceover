...
def test_voxcpm2_batch_voice_stage_uses_one_chapter_runner_slot(
    tmp_path: Path, monkeypatch
):
...
def test_batch_concurrency_configuration_has_safe_defaults_and_hard_ceilings(
    ...
    assert defaults.voxcpm_workers == 1
...
def test_direct_voxcpm2_tts_requests_share_one_local_voxcpm_slot(
    tmp_path: Path, monkeypatch
):
    monkeypatch.delenv("AUDIOBOOK_VOXCPM_WORKER_CONCURRENCY", raising=False)
    state = ServerState(tmp_path)
    ...
    try:
        ...
        # MiMo remains independent, while later VoxCPM2 requests wait for the
        # one shared local chapter permit.
        assert state.worker_resource_semaphores["mimo"].acquire(blocking=False)
        state.worker_resource_semaphores["mimo"].release()
        time.sleep(0.05)
        assert maximum == 1
